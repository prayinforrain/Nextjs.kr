...
Next.js는 불러온 파일에 기반하여 `width`와 `height`를 [자동으로 결정](#image-sizing)합니다. 이 값들은 이미지 로딩시 [누적 레이아웃 이동](https://nextjs.org/learn/seo/web-performance/cls)을 예방하는데 사용됩니다.
...
`width`와 `height` 어트리뷰트는 정확한 화면비를 추론하고 로딩 도중 레이아웃 이동을 피하기 위해 사용됩니다. `width`와 `height`는 렌더링된 이미지 파일의 크기를 정하지는 _않습니다_. 더 자세한 내용은 [Image Sizing](#image-sizing) 문서를 참조하세요.
...
Next.js 애플리케이션의 기본 로더는 모든 출처의 이미지를 최적화하여 Next.js 웹 서버에서 직접 서브하는 내장 이미지 최적화 API를 사용합니다. CDN이나 이미지 서버로부터 이미지를 서브하고 싶다면 JavaScript 몇 줄로 로더 함수를 직접 구현하면 가능합니다.
